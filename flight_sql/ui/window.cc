...
std::unique_ptr<Window> Window::CreateTabControl(int id)
{
    ...
    child->Create(WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | WS_TABSTOP, 0, 0, rcClient.right, 20, id);

    return child;
}

std::unique_ptr<Window> Window::CreateList(int posX, int posY,
    int sizeX, int sizeY, int id)
{
    std::unique_ptr<Window> child(new Window(this, WC_LISTVIEW, ""));

    child->Create(WS_CHILD | WS_VISIBLE | WS_BORDER | LVS_REPORT | LVS_EDITLABELS | WS_TABSTOP, posX, posY, sizeX, sizeY, id);

    return child;
}

std::unique_ptr<Window> Window::CreateGroupBox(int posX, int posY,
...
std::unique_ptr<Window> Window::CreateLabel(int posX, int posY,
    int sizeX, int sizeY, const char* title, int id)
{
...
std::unique_ptr<Window> Window::CreateEdit(int posX, int posY,
...
std::unique_ptr<Window> Window::CreateButton(int posX, int posY,
    int sizeX, int sizeY, const char* title, int id, int style)
{
    std::unique_ptr<Window> child(new Window(this, "Button", title));
...
std::unique_ptr<Window> Window::CreateCheckBox(int posX, int posY,
...
std::unique_ptr<Window> Window::CreateComboBox(int posX, int posY,
    int sizeX, int sizeY, const char* title, int id)
{
...
void Window::ListAddItem(const std::vector<std::string >& items) 
{
    ...
    for (size_t i = 1; i < items.size(); ++i) {
        ListView_SetItemText(handle, ret, static_cast<int>(i), const_cast<char*>(items[i].c_str()));
    }
}
