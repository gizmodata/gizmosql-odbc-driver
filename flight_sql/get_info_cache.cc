...
void GetInfoCache::SetProperty(
    uint16_t property, driver::odbcabstraction::Connection::Info value) {
...
Connection::Info GetInfoCache::GetInfo(uint16_t info_type) {
  ...
  if (info_.end() == it) {
    ...
    if (info_.end() == it) {
      throw DriverException("Unknown GetInfo type: " +
                            std::to_string(info_type));
    }
  }
  return it->second;
}
    ...
    while (chunk_iter.GetNext(&chunk)) {
      ...
      for (int64_t i = 0; i < chunk.data->num_rows(); ++i) {
        if (!value_array->IsNull(i)) {
          auto info_type =
              static_cast<arrow::flight::sql::SqlInfoOptions::SqlInfo>(
          ...
          case ARROW_SQL_MAX_ROW_SIZE: {
            info_[SQL_MAX_ROW_SIZE] =
                static_cast<uint32_t>(ScalarToInt64(scalar));
            break;
          }
          case SqlInfoOptions::SQL_MAX_STATEMENT_LENGTH: {
            info_[SQL_MAX_STATEMENT_LEN] =
                static_cast<uint32_t>(ScalarToInt64(scalar));
            break;
          }
            ...
            for (int64_t map_index = 0; map_index < map_contents->length();
              ...
              if (!map_values->IsNull(map_index)) {
                auto map_key_scalar_ptr =
                    map_keys->GetScalar(map_index).ValueOrDie();
                auto map_value_scalar_ptr =
                    map_values->GetScalar(map_index).ValueOrDie();
                int32_t map_key_scalar = reinterpret_cast<arrow::Int32Scalar *>(
                                             map_key_scalar_ptr.get())
                                             ->value;
                auto map_value_scalar =
                    reinterpret_cast<arrow::BaseListScalar *>(
