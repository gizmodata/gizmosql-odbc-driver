...
### Fixed
- Fix DDL/DML not executing when client doesn't fetch results (e.g., pyodbc `cursor.execute("CREATE TABLE ...")`): GizmoSQL uses lazy execution — `GetFlightInfo` only plans the query, and actual execution is deferred to `DoGet()`. For DDL/DML statements the server returns empty Flight endpoints, so if the client never calls `DoGet()`, the statement is never executed. The driver now detects empty endpoints and falls back to `ExecuteUpdate` (DoPut RPC) which executes immediately. Affects both direct execution (`SQLExecDirect`) and prepared statement execution (`SQLExecute`).
